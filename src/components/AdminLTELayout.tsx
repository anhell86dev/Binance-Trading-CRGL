...
import { atrAlertService, AtrAlertItem } from '../services/atrAlertService';
...
import { AtrCriticalTopAlertBar } from './AtrCriticalTopAlertBar';
import { AtrRiskManagementModal } from './AtrRiskManagementModal';
...
export const AdminLTELayout: React.FC<AdminLTELayoutProps> = ({
  ...
  // ATR Critical Alerts & Volatility state
  const [isAtrModalOpen, setIsAtrModalOpen] = useState(false);
  const [atrTargetSymbol, setAtrTargetSymbol] = useState<string | undefined>(undefined);
  const [activeAtrAlerts, setActiveAtrAlerts] = useState<AtrAlertItem[]>(() => atrAlertService.getActiveAlerts());
  const [currentAtrPct, setCurrentAtrPct] = useState<number>(() => {
    const curSym = binanceWs.getCurrentSymbol();
    const conf = advancedTechnicalConfluenceService.getConfluence(curSym);
    return conf?.atr14Percent || 2.1;
  });
  ...
  // Subscribe to ATR alert updates
  useEffect(() => {
    const updateAtr = () => {
      setActiveAtrAlerts(atrAlertService.getActiveAlerts());
      const curSym = binanceWs.getCurrentSymbol();
      const conf = advancedTechnicalConfluenceService.getConfluence(curSym);
      if (conf?.atr14Percent) {
        setCurrentAtrPct(conf.atr14Percent);
      }
    };
    const unsubAtr = atrAlertService.subscribe(updateAtr);
    const unsubConf = advancedTechnicalConfluenceService.subscribe(updateAtr);
    return () => {
      unsubAtr();
      unsubConf();
    };
  ...
  const menuCategories: MenuCategory[] = [
    ...
    },
    {
      ...
      items: [
        ...
        {
          id: 'atr-risk' as const,
          label: 'Gestión de Riesgo & ATR',
          shortLabel: 'Alerta ATR',
          icon: ShieldAlert,
          badge: activeAtrAlerts.length > 0 ? `${activeAtrAlerts.length} CRÍTICO` : `${currentAtrPct.toFixed(1)}%`,
          badgeColor: activeAtrAlerts.length > 0 ? 'bg-rose-500/20 text-rose-300 border border-rose-500/40 animate-pulse' : 'bg-neutral-800 text-amber-300 font-mono',
          category: 'Riesgo',
          isAction: true,
          action: () => {
            const highest = atrAlertService.getHighestCriticalAlert();
            setAtrTargetSymbol(highest ? highest.symbol : binanceWs.getCurrentSymbol());
            setIsAtrModalOpen(true);
          },
          description: 'Auditoría de volatilidad ATR, alertas sonoras, cálculo de Stop Loss y mitigación',
        },
        {
  ...
  return (
    <div id="adminlte-app-wrapper" className="min-h-screen w-full bg-[#0b0e11] text-[#eaecef] flex overflow-hidden font-sans">
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden bg-[#0b0e11]">
        ...
        <header className="h-14 bg-[#161a1e] border-b border-[#262c36] px-3 sm:px-4 flex items-center justify-between gap-2 shrink-0 z-30 shadow-sm">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            {/* Live ATR Volatility Monitor Pill */}
            <button
              type="button"
              id="adminlte-btn-atr-monitor"
              onClick={() => {
                const highest = atrAlertService.getHighestCriticalAlert();
                setAtrTargetSymbol(highest ? highest.symbol : binanceWs.getCurrentSymbol());
                setIsAtrModalOpen(true);
              }}
              className={`px-2 py-0.5 rounded text-[11px] font-mono font-bold border flex items-center gap-1.5 transition-all cursor-pointer ${
                activeAtrAlerts.length > 0
                  ? 'bg-rose-500/20 text-rose-300 border-rose-500/50 animate-pulse shadow-xs shadow-rose-950/50'
                  : currentAtrPct >= 3.0
                  ? 'bg-amber-500/15 text-amber-300 border-amber-500/40'
                  : 'bg-[#101317] text-neutral-300 border-[#262c36] hover:border-neutral-700'
              }`}
              title={`Monitor de Volatilidad ATR (14): ${currentAtrPct.toFixed(2)}% - Haz clic para auditar gestión de riesgo y umbrales`}
            >
              {activeAtrAlerts.length > 0 ? (
                <Flame className="w-3 h-3 text-rose-400 fill-rose-400 animate-bounce" />
              ) : (
                <Activity className="w-3 h-3 text-amber-400" />
              )}
              <span className="hidden sm:inline">ATR:</span>
              <span>{currentAtrPct.toFixed(1)}%</span>
              {activeAtrAlerts.length > 0 && (
        ...
        {/* 2.1.1 ATR CRITICAL VOLATILITY TOP ALERT BANNER (High Visibility) */}
        <AtrCriticalTopAlertBar
          onOpenRiskModal={(sym) => {
            setAtrTargetSymbol(sym);
            setIsAtrModalOpen(true);
          }}
          onOpenSettingsModal={() => {
            setAtrTargetSymbol(undefined);
            setIsAtrModalOpen(true);
          }}
          onNavigateToTrades={() => onSelectTab('gestion-trades')}
        />
      ...
      {/* ATR Volatility & Risk Management Modal */}
      <AtrRiskManagementModal
        isOpen={isAtrModalOpen}
        onClose={() => setIsAtrModalOpen(false)}
        targetSymbol={atrTargetSymbol}
        onNavigateToTrades={() => onSelectTab('gestion-trades')}
      />
