...
class BinanceWsEngine {
  ...
  public loadSimulationDemoData() {
    ...
    const zecEntry = Number((price * 0.985).toFixed(2));
    const zecQty = 1.0;
    const zecNotional = Number((zecEntry * zecQty).toFixed(2));
    const zecMargin = Number((zecNotional / 3).toFixed(2));
    const zecPnl = Number(((price - zecEntry) * zecQty).toFixed(2));

    this.positions = [
      // 1. ZECUSDT: High Confluence (8/10), High R:B (1:3.4)
      {
        symbol: 'ZECUSDT',
        positionAmt: zecQty,
        entryPrice: zecEntry,
        markPrice: price,
        unRealizedProfit: zecPnl,
        liquidationPrice: Number((zecEntry * (1 - 1 / 3 + 0.005)).toFixed(2)),
        leverage: 3,
        marginType: 'ISOLATED',
        isolatedMargin: zecMargin,
        notional: zecNotional,
        roePercent: Number(((zecPnl / zecMargin) * 100).toFixed(2)),
        takeProfit: Number((zecEntry * 1.085).toFixed(2)),
        stopLoss: Number((zecEntry * 0.975).toFixed(2)),
        strategyId: 'VVV-20260906-RANGO',
        updatedAt: Date.now() - 3600000,
      },
      // 2. SOLUSDT: Strong Confluence (6/10), Good R:B (1:2.4)
      {
        symbol: 'SOLUSDT',
        positionAmt: 2.5,
        entryPrice: 182.40,
        markPrice: 185.80,
        unRealizedProfit: 8.50,
        liquidationPrice: 135.20,
        leverage: 4,
        marginType: 'ISOLATED',
        isolatedMargin: 114.00,
        notional: 456.00,
        roePercent: 7.45,
        takeProfit: 198.50,
        stopLoss: 175.70,
        strategyId: 'SOL-BREAKOUT-SWING',
        updatedAt: Date.now() - 7200000,
      },
      // 3. ETHUSDT: Moderate Confluence (5/10), Fair R:B (1:1.6)
      {
        symbol: 'ETHUSDT',
        positionAmt: 0.15,
        entryPrice: 3420.00,
        markPrice: 3405.00,
        unRealizedProfit: -2.25,
        liquidationPrice: 2450.00,
        leverage: 3,
        marginType: 'ISOLATED',
        isolatedMargin: 171.00,
        notional: 513.00,
        roePercent: -1.32,
        takeProfit: 3525.00,
        stopLoss: 3355.00,
      ...
      // 4. BTCUSDT: Low Confluence (3/10), Missing SL (Risk Alert)
      {
        symbol: 'BTCUSDT',
        positionAmt: 0.008,
        entryPrice: 91200.00,
        markPrice: 90850.00,
        unRealizedProfit: -2.80,
        liquidationPrice: 65000.00,
        leverage: 3,
        marginType: 'ISOLATED',
        isolatedMargin: 243.20,
        notional: 729.60,
        roePercent: -1.15,
        takeProfit: 94500.00,
        stopLoss: 0, // No SL!
    ...
    this.openOrders = [
      {
        ...
        clientOrderId: `TP-ZECUSDT-MARKET`,
        symbol: 'ZECUSDT',
        side: 'SELL',
        type: 'TAKE_PROFIT_MARKET',
        price: 0,
        stopPrice: Number((zecEntry * 1.085).toFixed(2)),
        origQty: zecQty,
        executedQty: 0,
        status: 'NEW',
        timeInForce: 'GTC',
      ...
      },
      {
        ...
        clientOrderId: `SL-ZECUSDT-STOP`,
        symbol: 'ZECUSDT',
        side: 'SELL',
        type: 'STOP_MARKET',
        price: Number((zecEntry * 0.975).toFixed(2)),
        stopPrice: Number((zecEntry * 0.975).toFixed(2)),
        origQty: zecQty,
        executedQty: 0,
        status: 'NEW',
        timeInForce: 'GTC',
      ...
      },
      {
        ...
        clientOrderId: `TP-SOLUSDT-MARKET`,
        symbol: 'SOLUSDT',
        side: 'SELL',
        type: 'TAKE_PROFIT_MARKET',
        price: 0,
    ...
    this.tradeHistory = [
      {
        ...
        price: zecEntry,
        quantity: zecQty,
        notional: Number((zecEntry * zecQty).toFixed(2)),
        realizedPnl: 0,
        commission: Number((zecEntry * zecQty * 0.0004).toFixed(3)),
